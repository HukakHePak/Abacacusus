...
import './App.css';
import {Abacus, AbacusProps} from "./components/Abacus/Abacus";
...
function App() {
    ...
    function changeHandler(paramName: string) {
        return (parameter: number | string) => setParams({...params, [paramName]: parameter});
    }
        ...
        <div className="App" style={{backgroundImage: `url(${forestImg})`}}>
            <div className="wrapper">
                <div className="settings">
                    <SelectNum value={params.size}
                               onChange={changeHandler('size')}>
                        Cтержни
                    </SelectNum>

                    <SelectNum value={params.topBonesCount}
                               onChange={changeHandler('topBonesCount')}>
                        Верхние косточки
                    </SelectNum>

                    <SelectNum value={params.botBonesCount}
                               onChange={changeHandler('botBonesCount')}>
                        Нижние косточки
                    </SelectNum>

                    <SelectImg data={[abacusSkin, abacusSkin2]}
                               value={params.skin}
                               onChange={changeHandler('skin')}/>

                    <SelectImg data={[boneSkin2, boneSkin]}
                               value={params.bonesSkin}
                               onChange={changeHandler('bonesSkin')}/>
                </div>
                <div className="content" style={{borderColor: params.skin === abacusSkin2 ? '#682413' : ' #838383'}}>
                    <Abacus {...params} separator={separatorHeight}/>
                </div>
            </div>
        </div>
    );
